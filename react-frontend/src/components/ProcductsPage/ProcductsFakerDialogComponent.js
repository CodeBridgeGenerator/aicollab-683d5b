...
import procductsFakerFactory from "./procductsFakerFactory";
...
const ProcductsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = procductsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("procducts").create(elem));
...
export default connect(mapState, mapDispatch)(ProcductsFakerDialogComponent);
